					...
					<div id="mainContainer">
						<div class="row">
							...
							<div class="col-md-10 form-group">
								...
								<input type="text" class="form-control" name="protectorImpt"
									id="protectorImpt" data-rule="protectorImpt"
									placeholder="보호자의 특징 및 주의해야 할 사항을 적어주세요." required="required">
								<div class="validate"></div>
							</div>
						</div>
						<br />
						...
						<div class="row">
							...
							<div class="col-md-10 form-group">
								...
								<input type="text" class="form-control" name="patientName" id="patientName" value="두둥"
									data-rule="patientName" readonly style="background-color: #F5F5F5">
						...
						<div class="row">
							...
							<div class="col-md-10 form-group">
								...
								<input type="text" class="form-control" name="species" id="species"
									value="말티즈" data-rule="species" readonly style="background-color: #F5F5F5">
						...
						<div class="row">
							...
							<div class="col-md-10 form-group">
								...
								<input type="text" class="form-control" name="size" id="size" value="소형"
									data-rule="size" readonly style="background-color: #F5F5F5">
						...
						<div class="row">
							...
							<div class="col-md-10 form-group">
								<label for="age">나이</label> <input type="number" min="1"
									class="form-control" name="age" id="age" value="3"
									data-rule="age" readonly style="background-color: #F5F5F5">
						...
						<div class="row">
							...
							<div class="col-md-10 form-group">
								...
								<input type="radio" name="gender" id="Female" data-rule="gender" value="Female"
									checked onClick="return false;">
								<label for="Female">Female</label>
								<input type="radio" name="gender" id="Male" data-rule="gender" value="Male" readonly
						...
						<div class="row">
							...
							<div class="col-md-10 form-group">
								<label for="weight">몸무게</label> <input type="number" min="1"
									class="form-control" name="weight" step="0.1" id="weight"
									value="5" data-rule="weight" readonly style="background-color: #F5F5F5">
						...
						<div class="row">
							...
							<div class="col-md-10 form-group">
								...
								<div class="form-control">
									<div class="col-md-8 ">
										<label for="medicine_name">약품명</label> <input type="text"
											class="form-control" list="medicine_name_option"
										...
										<label for="medication_guide">복약안내</label> <input type="text"
											class="form-control" list="medication_guide_option"
											data-rule="text" required="required">
...
<script>
	...
	$(function () {
		$('#diagnosis_add').on('click', function () {
			...
			if (patientName == null || patientName.trim().length == 0) {
				$('#patientName').attr('style', 'border-color: #dc3545;');
				$('#patientName').val('');
				$('#warningMessage1').html('<div id="warning1"><b style="color: red;">환자 이름을 입력해주세요</b></div>');
				$('#patientName').focus();
			} else if (species == null || species.trim().length == 0) {
				$('#species').attr('style', 'border-color: #dc3545;');
				$('#species').val('');
				$('#warningMessage2').html('<div id="warning2"><b style="color: red;">환자의 품종을 입력해주세요</b></div>');
				$('#species').focus();
			} else if (size == null || size.trim().length == 0) {
				$('#size').attr('style', 'border-color: #dc3545;');
				$('#size').val('');
				$('#warningMessage3').html('<div id="warning3"><b style="color: red;">환자의 크기를 입력해주세요</b></div>');
				$('#size').focus();
			} else if (age == null || age.trim().length == 0) {
				$('#age').attr('style', 'border-color: #dc3545;');
				$('#age').val('');
				$('#warningMessage4').html('<div id="warning4"><b style="color: red;">환자의 나이를 입력해주세요</b></div>');
				$('#age').focus();
			} else if (weight == null || weight.trim().length == 0) {
				$('#weight').attr('style', 'border-color: #dc3545;');
				$('#weight').val('');
				$('#warningMessage5').html('<div id="warning5"><b style="color: red;">환자의 몸무게를 입력해주세요</b></div>');
				$('#weight').focus();
			} else if (visitDate == null || visitDate.trim().length == 0) {
				$('#visitDate').attr('style', 'border-color: #dc3545;');
				$('#visitDate').val('');
				$('#warningMessage6').html('<div id="warning6"><b style="color: red;">진료 일자를 입력해주세요</b></div>');
				$('#visitDate').focus();
			} else if (symptom == null || symptom.trim().length == 0) {
				$('#symptom').attr('style', 'border-color: #dc3545;');
				$('#symptom').val('');
	...
	$('#patientName').on('input', function() {
		if ($('#patientName').val() != '') {
			$('#patientName').attr('style', 'border-color : #ced4da;');
			$('#warning1').remove();
		}
	});
	...
	$('#visitDate').on('input', function() {
		if ($('#visitDate').val() != '') {
			$('#visitDate').attr('style', 'border-color : #ced4da;');
			$('#warning6').remove();
		}
	});
	$('#symptom').on('input', function() {
		if ($('#symptom').val() != '') {
			$('#symptom').attr('style', 'border-color : #ced4da;');
			$('#warning7').remove();
		}
	});
</script>
